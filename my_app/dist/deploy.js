...
const waitForFunds = (wallet) => Rx.firstValueFrom(wallet.state().pipe(Rx.tap((state) => {
    if (state.syncProgress) {
        console.log(`Sync progress: synced=${state.syncProgress.synced}, ` +
            `sourceGap=${state.syncProgress.lag.sourceGap}, ` +
            `applyGap=${state.syncProgress.lag.applyGap}`);
    }
...
async function main() {
    console.log("Midnight Privacy Bridge Deployment\n");
    ...
    try {
        ...
        const contractModulePath = path.join(contractPath, "managed", "Privacy_Bridge", "contract", "index.cjs");
        ...
        const zkConfigPath = path.join(contractPath, "managed", "Privacy_Bridge");
        const providers = {
            privateStateProvider: levelPrivateStateProvider({
                privateStateStoreName: "Privacy_Bridge-state",
            }),
