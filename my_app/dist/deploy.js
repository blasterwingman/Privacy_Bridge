...
const waitForFunds = (wallet) => Rx.firstValueFrom(wallet.state().pipe(Rx.tap((s) => {
    if (s.syncProgress) {
        const lag = s.syncProgress.lag;
        console.log(`Sync: synced=${s.syncProgress.synced} sourceGap=${lag.sourceGap} applyGap=${lag.applyGap}`);
    }
}), Rx.filter((s) => s.syncProgress?.synced === true), Rx.map((s) => {
...
async function buildWalletProviders(wallet) {
    ...
    return {
        ...
        async balanceTx(tx, newCoins) {
            const zswap = ZswapTransaction.deserialize(tx.serialize(getLedgerNetworkId()), getZswapNetworkId());
            const balanced = await wallet.balanceTransaction(zswap, newCoins);
            const proven = await wallet.proveTransaction(balanced);
            const ledgerTx = Transaction.deserialize(proven.serialize(getZswapNetworkId()), getLedgerNetworkId());
...
function prettyPrintBalances(all) {
...
function tryPrintLocalHistory(state) {
    const keys = Object.keys(state || {});
    const candidate = state?.history ?? state?.transactions ?? state?.txs;
    if (!candidate) {
        console.log("No local history available on wallet.state().history | transactions.");
        console.log("Tip: you can also query recent txs via the Indexer GraphQL using your address.");
        return;
    }
    ...
    for (const item of candidate.slice(-10).reverse()) {
        ...
        if (typeof item === "string") {
            ...
            console.log(`  - txId=${item.public.txId} height=${item.public.blockHeight ?? "?"}`);
        }
        else if (item?.txId) {
...
async function main() {
    ...
    try {
        ...
        const wallet = await WalletBuilder.buildFromSeed(CONFIG.indexer, CONFIG.indexerWS, CONFIG.proof, CONFIG.node, walletSeed, getZswapNetworkId(), "info");
        wallet.start();
        // Wait for sync
        ...
        while (running) {
            ...
            switch (choice.trim()) {
                ...
                case "2": {
                    ...
                    const amtStr = await rl.question("Amount (in smallest units, bigint): ");
                    ...
                    try {
                        ...
                        console.error("✖ Your installed @midnight-ntwrk/wallet package does not expose a simple native-transfer helper.\n" +
                            "  To implement a real send, you need the official transfer/build API from the Wallet SDK\n" +
