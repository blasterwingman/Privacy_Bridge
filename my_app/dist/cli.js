...
// ---------- Helpers ----------
async function waitSynced(wallet) {
    await Rx.firstValueFrom(wallet
        .state()
...
async function balanceAndProve(wallet, zswapTx) {
    const balancedZswap = await wallet.balanceTransaction(zswapTx, 
    /* newCoins */ []);
    const provedZswap = await wallet.proveTransaction(balancedZswap);
    return Transaction.deserialize(provedZswap.serialize(getZswapNetworkId()), getLedgerNetworkId());
}
// NOTE on "send":
...
function buildPaymentSkeleton(toShieldAddr, amount) {
    ...
    const b = new ZS.TransactionBuilder(getZswapNetworkId());
...
async function fetchRecentTxs(address, limit = 10) {
...
async function main() {
    ...
    try {
        ...
        console.log("\nConnecting & syncing…");
        const wallet = await WalletBuilder.buildFromSeed(TESTNET_CONFIG.indexer, TESTNET_CONFIG.indexerWS, TESTNET_CONFIG.proofServer, TESTNET_CONFIG.node, seed, getZswapNetworkId(), "info");
        wallet.start();
            ...
            switch (choice.trim()) {
                ...
                case "2": {
                    ...
                    try {
                        ...
                        console.log("\n✅ Sent!");
                        const txId = submitted?.public?.txId ?? submitted?.txId ?? String(submitted ?? "(unknown)");
                        const includedAt = submitted?.public?.blockHeight ?? submitted?.blockHeight ?? "pending";
                    ...
                    try {
                        ...
                        if (!items.length) {
                            ...
                            for (const t of items) {
                                console.log(`- ${t.txId} | block ${t.blockHeight} | ${t.direction} | amount ${t.amount}`);
                            }
                        }
                    }
                    catch (e) {
